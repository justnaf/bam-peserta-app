<x-app-layout>
    <x-slot name="header">
        <section class="flex items-center">
            <a href="{{ route('ownpaper.index') }}" class="text-gray-700 hover:text-gray-900">
        ...
        <div class="max-w-7xl mx-auto px-2">
            <div class="bg-white overflow-hidden shadow-sm rounded-lg relative mb-4">
                <div class="py-8 px-2 text-gray-900">
                    <h1 class="text-center font-bold text-lg mb-3">Form Karya Tulis</h1>
                    <div class="relative overflow-x-auto">
                        <form action="{{route('ownpaper.store')}}" method="POST">
                            @csrf
                            <div class="w-full px-1 mb-2">
                                ...
                                <input class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Analisis pemahaman kucing terhadap berbagai rasa whiskas " type="text" name="name" id="name" required />
                            </div>
                            <div class="w-full px-1 mb-2">
                                <label class="block uppercase tracking-wide text-gray-700 text-sm text-center font-bold mb-2" for="achieve_year">
                                    Tahun
                                </label>
                                <input class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="2023" type="text" name="publish_year" id="publish_year" required />
                            </div>
